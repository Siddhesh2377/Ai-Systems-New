...
// Escapes src into dst as JSON string contents (no surrounding quotes).
// Reserves 6 bytes of headroom to fit a single \uXXXX escape at the end.
void escape_into(char* dst, size_t dst_size, const char* src) {
  if (!dst || dst_size == 0) return;
  size_t out = 0;
  if (!src) { dst[0] = '\0'; return; }
  for (size_t i = 0; src[i] && out + 7 < dst_size; ++i) {
...
// Async-signal-unsafe in theory; in practice we only touch pre-allocated
// buffers and call open/write/close. We never enter malloc here.
void crash_handler(int sig, siginfo_t* /*info*/, void* /*ctx*/) {
  ...
  if (path[0] != '\0') {
    ...
    if (fd >= 0) {
      ...
      int n = snprintf(prefix, sizeof(prefix),
          ...
          sig, sig_name, now_ms());
      if (n > 0) (void)!write(fd, prefix, (size_t)n);

      size_t state_len = std::strlen(g_state_buf);
      if (state_len > 0) (void)!write(fd, g_state_buf, state_len);
      else               (void)!write(fd, "null", 4);

      const char* tail = ",\"last_error\":";
      (void)!write(fd, tail, std::strlen(tail));
      size_t err_len = std::strlen(g_error_buf);
      if (err_len > 0) (void)!write(fd, g_error_buf, err_len);
      else             (void)!write(fd, "null", 4);
  ...
  // Reraise with default handler so Android's tombstone path still kicks in.
  struct sigaction sa{};
...
void tn_error_set_last(int code, const char* category, const char* message) {
  ...
  // Snapshot the op state so a concurrent set_op doesn't shred the JSON.
