...
// Process multimodal prompt and convert to MultimodalPrompt structure
    ...
    LlmSession::LlmSession(std::string model_path, json config, json extra_config,
                           std::vector<std::string> history) : model_path_(std::move(model_path)),
                                                               config_(std::move(config)),
                                                               extra_config_(
                                                                       std::move(extra_config)) {
        max_new_tokens_ = config_.contains("max_new_tokens") ? config_["max_new_tokens"].get<int>()
                                                             : 2048;
        keep_history_ = !extra_config_.contains("keep_history") ||
                        extra_config_["keep_history"].get<bool>();
        ...
        if (!history.empty()) {
            for (int i = 0; i < history.size(); i++) {
                if (is_r1_) {
                    if (i % 2 == 0) {
                        history_.emplace_back("user",
                                              getUserString(history[i].c_str(), true, is_r1_));
                    } else {
                        history_.emplace_back("assistant", getR1AssistantString(history[i]));
                    }
                } else {
                    history_.emplace_back(i % 2 == 0 ? "user" : "assistant",
                                          i % 2 == 0 ? history[i] : deleteThinkPart(history[i]));
                }
            }
        }
    }
    ...
    const MNN::Transformer::LlmContext *LlmSession::Response(const std::string &prompt,
        ...
        mls::Utf8StreamProcessor processor(
                [&response_buffer, &on_progress, this](const std::string &utf8Char) {
                    ...
                    if (!is_eop) {
                        ...
                        if (is_r1_) {
                            ...
                            if (user_think_pos != std::string::npos) {
                                last_message.second.erase(user_think_pos,
                                                          std::string("<think>\n").length());
                            }
...
//#define USE_DEBUG_PROMOT
        ...
        if (multimodal_result.has_multimodal) {
            ...
            if (!multimodal_result.error_message.empty()) {
                MNN_ERROR("Multimodal processing errors: %s",
                          multimodal_result.error_message.c_str());
            }
    ...
    void LlmSession::SetWavformCallback(std::function<bool(const float *, size_t, bool)> callback) {
        if (llm_ != nullptr && callback != nullptr) {
            waveform.clear();
            llm_->setWavformCallback(
                    [this, callback = std::move(callback)](const float *ptr, size_t size,
                                                           bool last_chunk) {
#if DEBUG_SAVE_WAV
    ...
    void LlmSession::updateConfig(const std::string &config_json) {
        ...
        if (llm_) {
            ...
            MNN_DEBUG("LLM not initialized yet, config saved for later: %s",
                      current_config_.dump().c_str());
        }
    }
    ...
    LlmSession::ResponseWithHistory(const std::vector<PromptItem> &full_history,
        ...
        if (multimodal_result.has_multimodal) {
            ...
            if (!multimodal_result.error_message.empty()) {
                MNN_ERROR("ResponseWithHistory: Multimodal processing errors: %s",
                          multimodal_result.error_message.c_str());
            }
            llm_->response(multimodal_result.multimodal_prompt, &output_ostream, "<eop>", 1);
        } else {
            MNN_DEBUG(
                    "ResponseWithHistory: No multimodal content detected, using regular text API");
...
// Pure C++ benchmark implementation following llm_bench.cpp exactly
    LlmSession::BenchmarkResult
    LlmSession::runBenchmark(int backend, int threads, bool useMmap, int power, int precision,
                             int memory, int dynamicOption, int nPrompt, int nGenerate, int nRepeat,
                             bool kvCache, const BenchmarkCallback &callback) {
...
// Initialize benchmark result structure
    LlmSession::BenchmarkResult
    LlmSession::initializeBenchmarkResult(int nPrompt, int nGenerate, int nRepeat, bool kvCache) {
...
// Initialize LLM for benchmark and verify it's ready
    bool LlmSession::initializeLlmForBenchmark(BenchmarkResult &result,
...
// Report benchmark progress
    void LlmSession::reportBenchmarkProgress(int iteration, int nRepeat, int nPrompt, int nGenerate,
                                             const BenchmarkCallback &callback) {
        if (callback.onProgress) {
            ...
            if (iteration == 0) {
                ...
                progressInfo.statusMessage = "Running test " + std::to_string(iteration) + "/" +
                                             std::to_string(nRepeat) + " (prompt=" +
                                             std::to_string(nPrompt) + ", generate=" +
                                             std::to_string(nGenerate) + ")";
...
// Run KV cache test iteration
    bool LlmSession::runKvCacheTest(int iteration, int nPrompt, int nGenerate,
        ...
        auto context = llm->getContext();
        if (!context) {
            result.error_message = "Context became invalid after response in kv-cache test " +
        ...
        if (iteration > 0) { // Exclude the first performance value
            ...
            processBenchmarkResults(context->prefill_us, context->decode_us, start_time, end_time,
                                    iteration, nPrompt, nGenerate, result, callback, true);
        }
        return true;
    }
    ...
    bool LlmSession::runLlamaBenchTest(int iteration, int nPrompt, int nGenerate,
        ...
        if ((nPrompt > 0 && tokens.empty()) || tokens1.empty()) {
            result.error_message =
                    "Invalid token configuration for llama-bench test " + std::to_string(iteration);
            if (callback.onError) callback.onError(result.error_message);
            return false;
        }
        ...
        if (nPrompt > 0) {
            ...
            if (!context) {
                result.error_message =
                        "Context became invalid after prefill response in llama-bench test " +
                        std::to_string(iteration);
                if (callback.onError) callback.onError(result.error_message);
                return false;
            }
        ...
        if (nGenerate > 0) {
            ...
            if (!context) {
                result.error_message =
                        "Context became invalid after decode response in llama-bench test " +
                        std::to_string(iteration);
                if (callback.onError) callback.onError(result.error_message);
                return false;
            }
        ...
        if (iteration > 0) { // Exclude the first performance value
            ...
            processBenchmarkResults(prefill_us, decode_us, start_time, end_time, iteration, nPrompt,
                                    nGenerate, result, callback, false);
...
// Process and report benchmark results
    void LlmSession::processBenchmarkResults(int64_t prefillTime, int64_t decodeTime,
                                             ...
                                             BenchmarkResult &result,
                                             const BenchmarkCallback &callback, bool isKvCache) {
        ...
        float prefillSpeed = (prefillTime > 0 && nPrompt > 0) ? ((float) nPrompt /
                                                                 prefillTimeSeconds) : 0.0f;
        float decodeSpeed = (decodeTime > 0 && nGenerate > 0) ? ((float) nGenerate /
                                                                 decodeTimeSeconds) : 0.0f;
        ...
        snprintf(detailedMsg, sizeof(detailedMsg),
                 ...
                 nPrompt, nGenerate, detailedInfo.progress, iteration, result.repeat_count, nPrompt,
                 nGenerate, runTimeSeconds, prefillTimeSeconds, decodeTimeSeconds, prefillSpeed,
                 decodeSpeed);
