...
Java_com_dark_ai_1mnn_NativeMNNChat_initNative(JNIEnv *env, jobject thiz, jstring modelDir,
                                               jobject chat_history, jstring mergeConfigStr,
...
Java_com_dark_ai_1mnn_NativeMNNChat_submitNative(JNIEnv *env, jobject thiz, jlong llmPtr,
                                                 jstring inputStr, jboolean keepHistory,
...
// 新增：支持完整历史消息的JNI方法
JNIEXPORT jobject JNICALL
Java_com_dark_ai_1mnn_NativeMNNChat_submitFullHistoryNative(JNIEnv *env, jobject thiz, jlong llmPtr,
...
Java_com_dark_ai_1mnn_NativeMNNChat_resetNative(JNIEnv *env, jobject thiz, jlong object_ptr) {
...
Java_com_dark_ai_1mnn_NativeMNNChat_setWavformCallbackNative(JNIEnv *env, jobject thiz,
                                                             jlong instance_id, jobject listener) {
...
Java_com_dark_ai_1mnn_NativeMNNChat_getDebugInfoNative(JNIEnv *env, jobject thiz, jlong objecPtr) {
...
JNIEXPORT void JNICALL
Java_com_dark_ai_1mnn_NativeMNNChat_releaseNative(JNIEnv *env, jobject thiz, jlong objecPtr) {
...
Java_com_dark_ai_1mnn_NativeMNNChat_updateMaxNewTokensNative(JNIEnv *env, jobject thiz,
                                                             jlong llm_ptr, jint max_new_tokens) {
...
Java_com_dark_ai_1mnn_NativeMNNChat_updateSystemPromptNative(JNIEnv *env, jobject thiz,
                                                             jlong llm_ptr,
...
extern "C" JNIEXPORT void JNICALL
Java_com_dark_ai_1mnn_NativeMNNChat_updateAssistantPromptNative(JNIEnv *env, jobject thiz,
                                                                jlong llm_ptr,
...
extern "C" JNIEXPORT void JNICALL
Java_com_dark_ai_1mnn_NativeMNNChat_updateConfigNative(JNIEnv *env, jobject thiz, jlong llm_ptr,
...
extern "C" JNIEXPORT void JNICALL
Java_com_dark_ai_1mnn_NativeMNNChat_updateEnableAudioOutputNative(JNIEnv *env, jobject thiz,
                                                                  jlong llm_ptr, jboolean enable) {
    auto *llm = reinterpret_cast<mls::LlmSession *>(llm_ptr);
    if (llm) {
...
extern "C" JNIEXPORT jstring JNICALL
Java_com_dark_ai_1mnn_NativeMNNChat_getSystemPromptNative(JNIEnv *env, jobject thiz,
                                                          jlong llm_ptr) {
    auto *llm = reinterpret_cast<mls::LlmSession *>(llm_ptr);
    if (llm) {
...
extern "C" JNIEXPORT void JNICALL
Java_com_dark_ai_1mnn_NativeMNNChat_clearHistoryNative(JNIEnv *env, jobject thiz, jlong llm_ptr) {
    auto *llm = reinterpret_cast<mls::LlmSession *>(llm_ptr);
    if (llm) {
        llm->clearHistory();
    }
...
Java_com_dark_ai_1mnn_NativeMNNChat_runBenchmarkNative(JNIEnv *env, jobject thiz, jlong llmPtr,
                                                       jint backend, jint threads, jboolean useMmap,
                                                       jint power, jint precision, jint memory,
                                                       jint dynamicOption, jint nPrompt,
                                                       jint nGenerate, jint nRepeat,
                                                       jboolean kvCache, jobject testInstance,
                                                       jobject callback) {
    ...
    jmethodID onErrorMethod = env->GetMethodID(callbackClass, "onBenchmarkError",
    ...
    jmethodID getPrefillUsMethod = env->GetMethodID(testInstanceClass, "getPrefillUs",
                                                    "()Ljava/util/List;");
    if (env->ExceptionCheck()) {
        MNN_ERROR("JNI Exception: GetMethodID for getPrefillUs failed.");
        env->ExceptionDescribe();
        env->ExceptionClear();
    }
    jmethodID getDecodeUsMethod = env->GetMethodID(testInstanceClass, "getDecodeUs",
                                                   "()Ljava/util/List;");
    if (env->ExceptionCheck()) {
        MNN_ERROR("JNI Exception: GetMethodID for getDecodeUs failed.");
        env->ExceptionDescribe();
        env->ExceptionClear();
    }
    jmethodID getSamplesUsMethod = env->GetMethodID(testInstanceClass, "getSamplesUs",
                                                    "()Ljava/util/List;");
    if (env->ExceptionCheck()) {
        MNN_ERROR("JNI Exception: GetMethodID for getSamplesUs failed.");
        env->ExceptionDescribe();
        env->ExceptionClear();
    }
    ...
    if (env->ExceptionCheck()) {
        MNN_ERROR("JNI Exception: FindClass for java/util/List failed.");
        env->ExceptionDescribe();
        env->ExceptionClear();
    }
    jmethodID addMethod = env->GetMethodID(listClass, "add", "(Ljava/lang/Object;)Z");
    if (env->ExceptionCheck()) {
        MNN_ERROR("JNI Exception: GetMethodID for List.add failed.");
        env->ExceptionDescribe();
        env->ExceptionClear();
    }
    ...
    if (env->ExceptionCheck()) {
        MNN_ERROR("JNI Exception: CallObjectMethod for getPrefillUs failed.");
        env->ExceptionDescribe();
        env->ExceptionClear();
    }
    jobject decodeUsList = env->CallObjectMethod(testInstance, getDecodeUsMethod);
    if (env->ExceptionCheck()) {
        MNN_ERROR("JNI Exception: CallObjectMethod for getDecodeUs failed.");
        env->ExceptionDescribe();
        env->ExceptionClear();
    }
    jobject samplesUsList = env->CallObjectMethod(testInstance, getSamplesUsMethod);
    if (env->ExceptionCheck()) {
        MNN_ERROR("JNI Exception: CallObjectMethod for getSamplesUs failed.");
        env->ExceptionDescribe();
        env->ExceptionClear();
    }
    ...
    jclass longClass = env->FindClass("java/lang/Long");
    if (env->ExceptionCheck()) {
        MNN_ERROR("JNI Exception: FindClass for java/lang/Long failed.");
        env->ExceptionDescribe();
        env->ExceptionClear();
    }
    jmethodID longCtor = env->GetMethodID(longClass, "<init>", "(J)V");
    if (env->ExceptionCheck()) {
        MNN_ERROR("JNI Exception: GetMethodID for Long constructor failed.");
        env->ExceptionDescribe();
        env->ExceptionClear();
    }
    ...
    cppCallback.onProgress = [env, globalCallback, onProgressMethod, progressClass, progressCtor](
        ...
        if (onProgressMethod && progressClass && progressCtor) {
            jobject progressObj = env->NewObject(progressClass, progressCtor,
                                                 (jint) progressInfo.progress, env->NewStringUTF(
                            progressInfo.statusMessage.c_str()), (jint) progressInfo.progressType,
    ...
    cppCallback.onIterationComplete = [env, globalCallback, onProgressMethod, progressClass, progressCtor](
        ...
        if (onProgressMethod && progressClass && progressCtor) {
            ...
            jobject progressObj = env->NewObject(progressClass, progressCtor, (jint) -1,
                                                 env->NewStringUTF(
                                                         detailed_stats.c_str()), // -1 indicates detailed stats
                                                 (jint) 0, (jint) 0, (jint) 0, (jint) 0,
                                                 (jint) 0, // default values for structured fields
                                                 (jfloat) 0.0f, (jfloat) 0.0f, (jfloat) 0.0f,
                                                 (jfloat) 0.0f, (jfloat) 0.0f);
            env->CallVoidMethod(globalCallback, onProgressMethod, progressObj);
            env->DeleteLocalRef(progressObj);
        }
    };
    ...
    auto result = llm_session->runBenchmark((int) backend, (int) threads, (bool) useMmap,
                                            (int) power, (int) precision, (int) memory,
                                            (int) dynamicOption, (int) nPrompt, (int) nGenerate,
    ...
    MNN_DEBUG(
            "BENCHMARK_JNI: Handing off to Kotlin. Prefill times count: %d, Decode times count: %d",
            (int) result.prefill_times_us.size(), (int) result.decode_times_us.size());
    ...
    if (result.success) {
        if (!prefillUsList || !decodeUsList || !samplesUsList) {
            ...
            for (long prefill_time: result.prefill_times_us) {
                ...
                if (env->ExceptionCheck()) {
                    MNN_ERROR("JNI Exception after NewObject for prefill.");
                    break;
                }
                env->CallBooleanMethod(prefillUsList, addMethod, prefillLong);
                if (env->ExceptionCheck()) {
                    MNN_ERROR("JNI Exception after add for prefill.");
                    break;
                }
            ...
            for (long decode_time: result.decode_times_us) {
                ...
                if (env->ExceptionCheck()) {
                    MNN_ERROR("JNI Exception after NewObject for decode.");
                    break;
                }
                env->CallBooleanMethod(decodeUsList, addMethod, decodeLong);
                if (env->ExceptionCheck()) {
                    MNN_ERROR("JNI Exception after add for decode.");
                    break;
                }
            ...
            for (long sample_time: result.sample_times_us) {
                ...
                if (env->ExceptionCheck()) {
                    MNN_ERROR("JNI Exception after NewObject for sample.");
                    break;
                }
                env->CallBooleanMethod(samplesUsList, addMethod, sampleLong);
                if (env->ExceptionCheck()) {
                    MNN_ERROR("JNI Exception after add for sample.");
                    break;
                }
    ...
    jstring errorMessage = result.success ? nullptr : env->NewStringUTF(
            result.error_message.c_str());
    return env->NewObject(resultClass, resultCtor, testInstance, (jboolean) result.success,
                          errorMessage);
}
