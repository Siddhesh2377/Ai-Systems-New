...
std::string clipPath, unetPath, vaeDecoderPath, vaeEncoderPath, safetyCheckerPath, tokenizerPath, patchPath, modelDir, upscalerPath;
...
struct PatchedModelBuffer {
    ...
    PatchedModelBuffer(uint8_t *buf, uint64_t sz) : buffer(buf, std::default_delete<uint8_t[]>()),
                                                    size(sz) {}
...
std::unique_ptr<QnnModel>
createQnnModel(const std::string &modelPath, const std::string &modelName) {
    using namespace qnn::tools;
    QnnFunctionPointers funcs = g_qnnSystemFuncs;
    void *backendHandle = nullptr;
    void *modelHandle = nullptr;
    dynamicloadutil::StatusCode drvStatus = dynamicloadutil::getQnnFunctionPointers(
            g_backendPathCmd, modelPath, &funcs, &backendHandle, false, &modelHandle);
    ...
    return std::make_unique<QnnModel>(funcs, inputListPaths, opPackagePaths, backendHandle,
                                      outputPath, debug, outputDataType, inputDataType,
                                      profilingLevel, dumpOutputs, modelPath, saveBinaryName);
}
...
namespace qnn {
    namespace tools {
        namespace sample_app {
            ...
            void writeFileForPatch(const std::string &filePath, const std::vector<char> &data) {
            ...
            int applyZstdPatch(const std::string &oldFilePath, const std::string &patchFilePath,
                ...
                try {
                    ...
                    if (decompressedSize == ZSTD_CONTENTSIZE_ERROR) {
                        throw std::runtime_error(
                                "Patch file (" + patchFilePath + ") is not a valid zstd frame.");
                    }
                    ...
                    size_t const actualDecompressedSize = ZSTD_decompress_usingDict(dctx,
                                                                                    newFileBuffer.data(),
                                                                                    newFileBuffer.size(),
                                                                                    patchFileBuffer.data(),
                                                                                    patchFileBuffer.size(),
                                                                                    oldFileBuffer.data(),
                                                                                    oldFileBuffer.size());
                    ...
                    if (ZSTD_isError(actualDecompressedSize)) {
                        throw std::runtime_error("ZSTD_decompress_usingDict() failed: " +
                                                 std::string(ZSTD_getErrorName(
            ...
            std::unique_ptr<PatchedModelBuffer>
            applyZstdPatchToBuffer(const std::string &oldFilePath,
                ...
                try {
                    ...
                    size_t const actualDecompressedSize = ZSTD_decompress_usingDict(dctx, newBuffer,
                                                                                    decompressedSize,
                                                                                    patchFileBuffer.data(),
                                                                                    patchFileBuffer.size(),
                                                                                    oldFileBuffer.data(),
                    ...
                    return std::make_unique<PatchedModelBuffer>(newBuffer, actualDecompressedSize);
...
// QnnModel Initialization
            ...
            int initializeQnnApp(const std::string &modelName, std::unique_ptr<AppType> &app,
...
// Command line processing
            void processCommandLine(int argc, char **argv) {
                ...
                static struct pal::Option s_longOptions[] = {{"help",                pal::no_argument, NULL,       OPT_HELP},
                        ...
                        case OPT_CLIP:
                            ...
                            if (clipPath.length() >= 8 &&
                                ...
                                if (std::filesystem::exists(v2Path)) {
                                    ...
                                    std::filesystem::path tokenEmbPath =
                                            parentDir / "token_emb.bin";

                                    if (!std::filesystem::exists(posEmbPath))
                                        showHelpAndExit(
                                                "pos_emb.bin not found: " + posEmbPath.string());
                ...
                if (upscaler_mode) {
                    ...
                    dynamicloadutil::StatusCode sysStatus = dynamicloadutil::getQnnSystemFunctionPointers(
                            systemLibraryPathCmd, &g_qnnSystemFuncs);
                ...
                if (use_safety_checker) {
                    safetyCheckerInterpreter = MNN::Interpreter::createFromFile(
                            safetyCheckerPath.c_str());
...
ProcessedPrompt processWeightedPrompt(const std::string &prompt_text, int max_len = 77) {
...
ProcessedPromptPair
processPromptPair(const std::string &positive, const std::string &negative, int max_len = 77) {
    ...
    result.ids.insert(result.ids.end(), neg_result.ids.begin(), neg_result.ids.end());
    result.ids.insert(result.ids.end(), pos_result.ids.begin(), pos_result.ids.end());
...
xt::xarray<float> blend_vae_encoder_tiles(
        ...
        const std::vector<std::pair<int, int>> &positions, int latent_h, int latent_w,
        int tile_size, int overlap) {
    if (tiles_mean_std.empty()) {
        throw std::runtime_error("Tile list cannot be empty for VAE encoder blending.");
    }
    ...
    for (size_t idx = 0; idx < tiles_mean_std.size(); ++idx) {
        ...
        const auto &mean_tile = tiles_mean_std[idx].first;  // (1, 4, tile_size, tile_size)
        const auto &std_tile = tiles_mean_std[idx].second;  // (1, 4, tile_size, tile_size)

        for (int c = 0; c < 4; ++c) {
            auto acc_mean_slice = xt::view(accumulated_mean, 0, c, xt::range(y, y + tile_size),
                                           xt::range(x, x + tile_size));
            auto mean_slice = xt::view(mean_tile, 0, c, xt::all(), xt::all());
            acc_mean_slice += mean_slice * tile_weight;

            auto acc_std_slice = xt::view(accumulated_std, 0, c, xt::range(y, y + tile_size),
                                          xt::range(x, x + tile_size));
    ...
    xt::xarray<float> weight_expanded = xt::reshape_view(weight_map, {1, 1, latent_h, latent_w});

    xt::xarray<float> final_mean = accumulated_mean / weight_expanded;
    xt::xarray<float> final_std = accumulated_std / weight_expanded;

    xt::xarray<float> noise = xt::random::randn<float>({1, 4, latent_h, latent_w});
    xt::xarray<float> latent = xt::eval(final_mean + final_std * noise);

    return latent;
}

xt::xarray<float> blend_vae_output_tiles(const std::vector<xt::xarray<float>> &tiles,
                                         const std::vector<std::pair<int, int>> &positions,
                                         int output_h, int output_w, int tile_size, int overlap) {
    if (tiles.empty()) {
        throw std::runtime_error("Tile list cannot be empty for VAE output blending.");
    }
    ...
    weight_map = xt::maximum(weight_map, 1e-8f);
    xt::xarray<float> weight_expanded = xt::reshape_view(weight_map, {1, 1, output_h, output_w});

    return accumulated / weight_expanded;
}

// --- Upscaler Tiling ---
std::vector<int> calculate_tile_positions(int dimension, int tile_size, int min_overlap) {
    ...
    if (dimension > tile_size) {
        num_tiles += (dimension - tile_size + effective_tile_size - 1) / effective_tile_size;
    }
...
xt::xarray<uint8_t>
upscaleImageWithModel(const std::vector<uint8_t> &input_image, int width, int height,
    ...
    QNN_INFO("Upscaling %dx%d to %dx%d using %dx%d tiles (variable overlap)", width, height,
             output_width, output_height, num_tiles_w, num_tiles_h);
    ...
    xt::xarray<float> input_chw = xt::transpose(input_hwc_f32, {0, 3, 1, 2});  // (1, 3, H, W)

    std::vector<int> output_shape = {1, 3, output_height, output_width};
    xt::xarray<float> accumulated_output = xt::zeros<float>(output_shape);
    xt::xarray<float> weight_map = xt::zeros<float>({output_height, output_width});

    int output_overlap = min_overlap * scale_factor;
    int fade_size = output_overlap / 2;
    xt::xarray<float> tile_weight = xt::ones<float>({output_tile_size, output_tile_size});
    ...
    for (int y: y_coords) {
        for (int x: x_coords) {
            xt::xarray<float> input_tile = xt::view(input_chw, 0, xt::all(),
                                                    xt::range(y, y + tile_size),
                                                    xt::range(x, x + tile_size));

            std::vector<float> tile_input_vec(input_tile.begin(), input_tile.end());
            std::vector<float> tile_output_vec(1 * 3 * output_tile_size * output_tile_size);

            if (StatusCode::SUCCESS !=
                upscaler->executeUpscalerGraphs(tile_input_vec.data(), tile_output_vec.data())) {
                throw std::runtime_error("Upscaler execution failed for tile");
            }

            std::vector<int> tile_output_shape = {1, 3, output_tile_size, output_tile_size};
            xt::xarray<float> output_tile = xt::adapt(tile_output_vec, tile_output_shape);
            ...
            auto weight_slice = xt::view(weight_map, xt::range(out_y, out_y + output_tile_size),
                                         xt::range(out_x, out_x + output_tile_size));
            weight_slice += tile_weight;

            tile_count++;
            std::cout << "Processed tile " << tile_count << "/" << (num_tiles_w * num_tiles_h)
                      << std::endl;
        }
    }

    weight_map = xt::maximum(weight_map, 1e-8f);
    xt::xarray<float> weight_expanded = xt::reshape_view(weight_map,
...
std::tuple<std::vector<std::pair<int, int>>, std::vector<std::pair<int, int>>, int, int>
calculate_vae_tile_positions(int pixel_width, int pixel_height) {
    ...
    auto pixel_x_coords = calculate_tile_positions(pixel_width, vae_tile_size,
                                                   min_latent_overlap * scale_factor);
    auto pixel_y_coords = calculate_tile_positions(pixel_height, vae_tile_size,
                                                   min_latent_overlap * scale_factor);
    ...
    if (pixel_x_coords.size() > 1) {
        ...
        latent_overlap = vae_latent_tile_size - (latent_x_coords[1] - latent_x_coords[0]);
        ...
        latent_overlap = vae_latent_tile_size - (latent_y_coords[1] - latent_y_coords[0]);
    }
...
xt::xarray<uint8_t>
upscaleImageWithMNN(const std::vector<uint8_t> &input_image, int width, int height,
    ...
    if (!interpreter) {
        throw std::runtime_error("Failed to create MNN interpreter from: " + model_path);
    }
    ...
    QNN_INFO("Upscaling %dx%d to %dx%d using MNN (%s), %dx%d tiles", width, height, output_width,
             output_height, use_opencl ? "OpenCL" : "CPU", num_tiles_w, num_tiles_h);
    ...
    for (int y: y_coords) {
        for (int x: x_coords) {
            ...
            auto host_tensor = MNN::Tensor::create<float>(dims,
                                                          const_cast<float *>(input_tile.data()),
                                                          MNN::Tensor::CAFFE);
            ...
            // Get output
            auto output_host = MNN::Tensor::create<float>(
                    {1, 3, output_tile_size, output_tile_size}, nullptr, MNN::Tensor::CAFFE);
            output_tensor->copyToHostTensor(output_host);

            std::vector<int> tile_output_shape = {1, 3, output_tile_size, output_tile_size};
            xt::xarray<float> output_tile = xt::adapt(output_host->host<float>(),
                                                      output_tile_size * output_tile_size * 3,
...
// --- Image Generation ---
GenerationResult generateImage(std::function<void(int step, int total_steps)> progress_callback) {
    ...
    if (request_has_mask && (mask_data.size() != 4 * sample_width * sample_height ||
    ...
    try {
        ...
        // --- CLIP ---
        ProcessedPromptPair processed = processPromptPair(prompt, negative_prompt, 77);
        ...
        std::vector<float> text_embedding_float(batch_size * 77 * text_embedding_size);
        ...
        if (use_mnn || use_mnn_clip) {
            ...
            if (use_mnn_clip) {
                ...
                if (!currentClipInterpreter)
                    throw std::runtime_error("Global clipInterpreter (hybrid) not initialized!");
            } else {
                currentClipInterpreter = MNN::Interpreter::createFromFile(clipPath.c_str());
                if (!currentClipInterpreter)
                    throw std::runtime_error("Failed to create temporary MNN CLIP interpreter!");
                dynamicCreated = true;
            }
            ...
            if (use_clip_v2) {
                ...
                auto out = currentClipInterpreter->getSessionOutput(currentClipSession,
                                                                    "last_hidden_state");
                memcpy(embed_ptr, out->host<float>(), 77 * text_embedding_size * sizeof(float));
            ...
            if (StatusCode::SUCCESS != clipApp->executeClipGraphs(input_ids_ptr, embed_ptr))
                throw std::runtime_error("QNN CLIP exec failed (neg)");
            if (StatusCode::SUCCESS != clipApp->executeClipGraphs(input_ids_ptr + 77, embed_ptr +
                                                                                      77 *
                                                                                      text_embedding_size))
        ...
        std::cout << "CLIP dur: " << std::chrono::duration_cast<std::chrono::milliseconds>(
                clip_end - clip_start).count() << "ms\n";
        current_step++;
        ...
        if (scheduler_type == "euler_a" || scheduler_type == "eulera") {
            scheduler = std::make_unique<EulerAncestralDiscreteScheduler>(1000, 0.00085f, 0.012f,
                                                                          "scaled_linear",
                                                                          "epsilon", "leading");
        } else {
            // Default to DPM solver
            scheduler = std::make_unique<DPMSolverMultistepScheduler>(1000, 0.00085f, 0.012f,
                                                                      "scaled_linear", 2, "epsilon",
                                                                      "leading");
        }
        ...
        std::vector<int> shape_batch2 = {batch_size, 4, sample_height, sample_width};
        xt::random::seed(seed);
        ...
        if (request_img2img) {
            ...
            bool need_vae_enc_tiling = ((output_width > 512 || output_height > 512) && !use_mnn &&
                                        vaeEncoderApp);
            ...
            if (!need_vae_enc_tiling) {
                ...
                if (use_mnn) {
                    MNN::Interpreter *currentVaeEncoderInterpreter = MNN::Interpreter::createFromFile(
                            vaeEncoderPath.c_str());
                    ...
                    if (use_opencl) {
                        auto cache_file =
                                modelDir + "/vae_enc_cache.mnnc." + std::to_string(output_width);
                    ...
                    MNN::Session *currentVaeEncSession = currentVaeEncoderInterpreter->createSession(
                            cfg_vae_enc);
                    if (!currentVaeEncSession)
                        throw std::runtime_error("Failed create temp MNN VAE Enc session!");

                    auto input = currentVaeEncoderInterpreter->getSessionInput(currentVaeEncSession,
                                                                               "input");
                    currentVaeEncoderInterpreter->resizeTensor(input,
                    ...
                    if (StatusCode::SUCCESS !=
                        vaeEncoderApp->executeVaeEncoderGraphs(img_data.data(), vae_enc_mean.data(),
                                                               vae_enc_std.data()))
                ...
                std::cout << "Using VAE encoder tiling for " << output_width << "x" << output_height
                          << " input..." << std::endl;
                ...
                auto [img_positions, latent_positions, img_overlap, latent_overlap] = calculate_vae_tile_positions(
                        output_width, output_height);

                int num_tiles = img_positions.size();
                std::cout << "VAE encoder will use " << num_tiles << " tiles with overlap "
                          << img_overlap << "px (latent: " << latent_overlap << ")" << std::endl;
                ...
                std::vector<std::pair<xt::xarray<float>, xt::xarray<float>>> encoded_tiles_mean_std;
                encoded_tiles_mean_std.reserve(img_positions.size());

                for (size_t i = 0; i < img_positions.size(); ++i) {
                    auto img_pos = img_positions[i];
                    xt::xarray<float> img_tile = xt::view(original_image, 0, xt::all(),
                                                          xt::range(img_pos.second, img_pos.second +
                                                                                    vae_enc_tile_size),
                                                          xt::range(img_pos.first, img_pos.first +
                                                                                   vae_enc_tile_size));

                    std::vector<float> tile_img_vec(img_tile.begin(), img_tile.end());
                    std::vector<float> tile_mean_vec(
                            1 * 4 * vae_enc_latent_tile_size * vae_enc_latent_tile_size);
                    std::vector<float> tile_std_vec(
                            1 * 4 * vae_enc_latent_tile_size * vae_enc_latent_tile_size);
                ...
                xt::xarray<float> img_lat = blend_vae_encoder_tiles(encoded_tiles_mean_std,
                                                                    latent_positions, sample_height,
                                                                    sample_width,
                                                                    vae_enc_latent_tile_size,
                                                                    latent_overlap);
            ...
            std::cout << "VAE Enc dur: " << std::chrono::duration_cast<std::chrono::milliseconds>(
                    vae_enc_end - vae_enc_start).count() << "ms\n";
            ...
            if (request_has_mask) {
                ...
                mask_full = xt::adapt(mask_data_full, {1, 3, output_height, output_width});
            }
        ...
        if (use_mnn) {
            currentUnetInterpreter = MNN::Interpreter::createFromFile(unetPath.c_str());
            if (!currentUnetInterpreter)
                throw std::runtime_error("Failed to create temporary MNN UNET interpreter!");

            MNN::ScheduleConfig cfg_unet;
            MNN::BackendConfig bkCfg_unet;
            if (use_opencl) {
                auto cache_file = modelDir + "/unet_cache.mnnc." + std::to_string(output_width);
            ...
            auto samp = currentUnetInterpreter->getSessionInput(currentUnetSession, "sample");
            auto ts = currentUnetInterpreter->getSessionInput(currentUnetSession, "timestep");
            auto enc = currentUnetInterpreter->getSessionInput(currentUnetSession,
                                                               "encoder_hidden_states");

            currentUnetInterpreter->resizeTensor(samp,
                                                 {batch_size, 4, sample_height, sample_width});
            currentUnetInterpreter->resizeTensor(ts, {1});
            currentUnetInterpreter->resizeTensor(enc, {batch_size, 77, text_embedding_size});
        ...
        for (int i = start_step; i < timesteps.size(); ++i) {
            ...
            xt::xarray<float> latents_scaled = scheduler->scale_model_input(latents, current_ts);
            ...
            if (use_mnn) {
                ...
                auto output = currentUnetInterpreter->getSessionOutput(currentUnetSession,
                                                                       "out_sample");
                ...
                if (StatusCode::SUCCESS !=
                    unetApp->executeUnetGraphs(latents_in_ptr, static_cast<int>(current_ts),
            ...
            xt::xarray<float> noise_pred_batch = xt::adapt(unet_out_latents, shape_batch2);
            ...
            if (request_has_mask) {
                ...
                xt::xarray<float> orig_noised = scheduler->add_noise(original_latents,
                                                                     latents_noise, t_xt);
        ...
        bool need_vae_tiling = ((output_width > 512 || output_height > 512) && !use_mnn);
        if (need_vae_tiling) {
            std::cout << "Using VAE decoder tiling for " << output_width << "x" << output_height
                      << " output..." << std::endl;
        }
        ...
        if (!need_vae_tiling) {
            ...
            std::vector<float> vae_dec_out_pixels(1 * 3 * output_width * output_height);

            if (use_mnn) {
                MNN::Interpreter *currentVaeDecoderInterpreter = MNN::Interpreter::createFromFile(
                ...
                MNN::Session *currentVaeDecSession = currentVaeDecoderInterpreter->createSession(
                        cfg_vae);

                if (!currentVaeDecSession)
                    throw std::runtime_error("Failed create temp MNN VAE Dec session!");

                auto input = currentVaeDecoderInterpreter->getSessionInput(currentVaeDecSession,
                                                                           "latent_sample");

                currentVaeDecoderInterpreter->resizeTensor(input,
                                                           {1, 4, sample_height, sample_width});
                ...
                auto input_nchw_tensor = new MNN::Tensor(input, MNN::Tensor::CAFFE);
                auto output = currentVaeDecoderInterpreter->getSessionOutput(currentVaeDecSession,
                                                                             "sample");
            ...
            // Use generic tile position calculator
            auto [output_positions, latent_positions, overlap, latent_overlap] = calculate_vae_tile_positions(
                    output_width, output_height);

            int num_tiles = output_positions.size();
            std::cout << "VAE decoder will use " << num_tiles << " tiles with overlap " << overlap
            ...
            for (size_t i = 0; i < latent_positions.size(); ++i) {
                ...
                xt::xarray<float> latent_tile = xt::view(latents, 0, xt::all(),
                                                         xt::range(lat_pos.second, lat_pos.second +
                                                                                   vae_latent_tile_size),
                                                         xt::range(lat_pos.first, lat_pos.first +
                                                                                  vae_latent_tile_size));

                std::vector<float> tile_latent_vec(latent_tile.begin(), latent_tile.end());
                std::vector<float> tile_output_vec(1 * 3 * vae_tile_size * vae_tile_size);
                ...
                std::cout << "Processed VAE tile " << i + 1 << "/" << latent_positions.size()
                          << std::endl;
            }
            ...
            pixels = blend_vae_output_tiles(decoded_tiles, output_positions, output_height,
        ...
        std::cout << "VAE Dec dur: " << std::chrono::duration_cast<std::chrono::milliseconds>(
                vae_dec_end - vae_dec_start).count() << "ms\n";
        ...
        if (request_has_mask) {
            ...
            auto blended = laplacianPyramidBlend(orig_img_view, gen_img_view, mask_view);
        ...
        if (use_safety_checker) {
            ...
            if (safety_check(out_data, output_width, output_height, score, safetyCheckerInterpreter,
            ...
            std::cout << "Safety check dur: "
                      << std::chrono::duration_cast<std::chrono::milliseconds>(
                              safety_end - safety_start).count() << "ms\n";
        }
        ...
        return GenerationResult{out_data, output_width, output_height, 3,
                                static_cast<int>(total_time), first_step_time_ms};
    } catch (const std::exception &e) {
...
Java_com_dark_ai_1sd_NativeLibStableDiffusion_initialize(JNIEnv *env, jobject thiz, jstring jClipPath,
                                                jstring jUnetPath, jstring jVaeDecoderPath,
                                                jstring jVaeEncoderPath, jstring jTokenizerPath,
                                                jstring jBackendPath, jstring jSystemLibraryPath,
                                                jstring jSafetyCheckerPath, jstring jPatchPath,
                                                jint jPort, jboolean jUseMnn, jboolean jUseMnnClip,
                                                jboolean jPonyv55, jboolean jUseSafetyChecker,
                                                jboolean jUpscalerMode) {
    using namespace qnn::tools;
    ...
    const char *clipPathStr = env->GetStringUTFChars(jClipPath, nullptr);
    const char *unetPathStr = env->GetStringUTFChars(jUnetPath, nullptr);
    const char *vaeDecoderPathStr = env->GetStringUTFChars(jVaeDecoderPath, nullptr);
    const char *vaeEncoderPathStr = jVaeEncoderPath ? env->GetStringUTFChars(jVaeEncoderPath,
                                                                             nullptr) : nullptr;
    const char *tokenizerPathStr = env->GetStringUTFChars(jTokenizerPath, nullptr);
    const char *backendPathStr = jBackendPath ? env->GetStringUTFChars(jBackendPath, nullptr)
                                              : nullptr;
    const char *systemLibraryPathStr = jSystemLibraryPath ? env->GetStringUTFChars(
            jSystemLibraryPath, nullptr) : nullptr;
    const char *safetyCheckerPathStr = jSafetyCheckerPath ? env->GetStringUTFChars(
            jSafetyCheckerPath, nullptr) : nullptr;
    const char *patchPathStr = jPatchPath ? env->GetStringUTFChars(jPatchPath, nullptr) : nullptr;

    clipPath = clipPathStr;
    unetPath = unetPathStr;
    vaeDecoderPath = vaeDecoderPathStr;
    if (vaeEncoderPathStr) vaeEncoderPath = vaeEncoderPathStr;
    tokenizerPath = tokenizerPathStr;
    if (safetyCheckerPathStr) safetyCheckerPath = safetyCheckerPathStr;
    if (patchPathStr) patchPath = patchPathStr;

    port = jPort;
    use_mnn = jUseMnn;
    use_mnn_clip = jUseMnnClip;
    ponyv55 = jPonyv55;
    use_safety_checker = jUseSafetyChecker;
    upscaler_mode = jUpscalerMode;

    modelDir = std::filesystem::path(clipPath).parent_path().string();

    // Check for clip_v2.mnn upgrade
    if (clipPath.length() >= 8 && clipPath.substr(clipPath.length() - 8) == "clip.mnn") {
        std::filesystem::path clipPathObj(clipPath);
        std::filesystem::path parentDir = clipPathObj.parent_path();
        std::filesystem::path v2Path = parentDir / "clip_v2.mnn";
        ...
        if (std::filesystem::exists(v2Path)) {
            ...
            if (std::filesystem::exists(posEmbPath) && std::filesystem::exists(tokenEmbPath)) {
                std::ifstream posFile(posEmbPath, std::ios::binary);
                posFile.seekg(0, std::ios::end);
                size_t posSize = posFile.tellg() / sizeof(float);
                posFile.seekg(0, std::ios::beg);
    ...
    if (!upscaler_mode) {
        try {
            ...
            if (!tokenizer) {
                env->ReleaseStringUTFChars(jClipPath, clipPathStr);
                env->ReleaseStringUTFChars(jUnetPath, unetPathStr);
                env->ReleaseStringUTFChars(jVaeDecoderPath, vaeDecoderPathStr);
                if (vaeEncoderPathStr)
                    env->ReleaseStringUTFChars(jVaeEncoderPath, vaeEncoderPathStr);
                env->ReleaseStringUTFChars(jTokenizerPath, tokenizerPathStr);
                if (backendPathStr) env->ReleaseStringUTFChars(jBackendPath, backendPathStr);
                if (systemLibraryPathStr)
                    env->ReleaseStringUTFChars(jSystemLibraryPath, systemLibraryPathStr);
                if (safetyCheckerPathStr)
                    env->ReleaseStringUTFChars(jSafetyCheckerPath, safetyCheckerPathStr);
                if (patchPathStr) env->ReleaseStringUTFChars(jPatchPath, patchPathStr);
                return EXIT_FAILURE;
            }
        } catch (const std::exception &e) {
            env->ReleaseStringUTFChars(jClipPath, clipPathStr);
            env->ReleaseStringUTFChars(jUnetPath, unetPathStr);
            env->ReleaseStringUTFChars(jVaeDecoderPath, vaeDecoderPathStr);
            if (vaeEncoderPathStr) env->ReleaseStringUTFChars(jVaeEncoderPath, vaeEncoderPathStr);
            env->ReleaseStringUTFChars(jTokenizerPath, tokenizerPathStr);
            if (backendPathStr) env->ReleaseStringUTFChars(jBackendPath, backendPathStr);
            if (systemLibraryPathStr)
                env->ReleaseStringUTFChars(jSystemLibraryPath, systemLibraryPathStr);
        ...
        if (use_safety_checker && safetyCheckerPath.length() > 0) {
            safetyCheckerInterpreter = MNN::Interpreter::createFromFile(safetyCheckerPath.c_str());
        }
        ...
        if (!use_mnn) {
            if (!systemLibraryPathStr || !backendPathStr) {
                env->ReleaseStringUTFChars(jClipPath, clipPathStr);
                env->ReleaseStringUTFChars(jUnetPath, unetPathStr);
                env->ReleaseStringUTFChars(jVaeDecoderPath, vaeDecoderPathStr);
                if (vaeEncoderPathStr)
            ...
            g_backendPathCmd = backendPathStr;
            dynamicloadutil::StatusCode sysStatus = dynamicloadutil::getQnnSystemFunctionPointers(
                    systemLibraryPathStr, &g_qnnSystemFuncs);
            if (sysStatus != dynamicloadutil::StatusCode::SUCCESS) {
                env->ReleaseStringUTFChars(jClipPath, clipPathStr);
                env->ReleaseStringUTFChars(jUnetPath, unetPathStr);
                env->ReleaseStringUTFChars(jVaeDecoderPath, vaeDecoderPathStr);
            ...
            if (!patchPath.empty()) {
                g_unetPatchedBuffer = sample_app::applyZstdPatchToBuffer(unetPath, patchPath);
                if (!g_unetPatchedBuffer) {
                    env->ReleaseStringUTFChars(jClipPath, clipPathStr);
                    env->ReleaseStringUTFChars(jUnetPath, unetPathStr);
                    env->ReleaseStringUTFChars(jVaeDecoderPath, vaeDecoderPathStr);
        ...
        if (safetyCheckerInterpreter) {
            ...
            if (safetyCheckerSession) {
                auto input = safetyCheckerInterpreter->getSessionInput(safetyCheckerSession,
                                                                       nullptr);
            ...
            if (unetApp) {
                if (g_unetPatchedBuffer && g_unetPatchedBuffer->buffer) {
                    status = sample_app::initializeQnnApp("UNET", unetApp,
                                                          g_unetPatchedBuffer->buffer.get(),
                                                          g_unetPatchedBuffer->size);
                } else {
                    status = sample_app::initializeQnnApp("UNET", unetApp);
                }
    ...
    env->ReleaseStringUTFChars(jTokenizerPath, tokenizerPathStr);
    if (backendPathStr) env->ReleaseStringUTFChars(jBackendPath, backendPathStr);
    if (systemLibraryPathStr) env->ReleaseStringUTFChars(jSystemLibraryPath, systemLibraryPathStr);
    if (safetyCheckerPathStr) env->ReleaseStringUTFChars(jSafetyCheckerPath, safetyCheckerPathStr);
    if (patchPathStr) env->ReleaseStringUTFChars(jPatchPath, patchPathStr);
...
Java_com_dark_ai_1sd_NativeLibStableDiffusion_cleanup(JNIEnv *env, jobject thiz) {
    if (clipSession) clipInterpreter->releaseSession(clipSession);
    clipSession = nullptr;
    if (unetSession) unetInterpreter->releaseSession(unetSession);
    unetSession = nullptr;
    if (safetyCheckerSession) safetyCheckerInterpreter->releaseSession(safetyCheckerSession);
...
JNIEXPORT void JNICALL
Java_com_dark_ai_1sd_NativeLibStableDiffusion_startServer(JNIEnv *env, jobject thiz) {
// HTTP Server setup (same as in main)
    httplib::Server svr;
...
// Add all the POST endpoints from main() here...
// /generate endpoint
// /upscale endpoint
