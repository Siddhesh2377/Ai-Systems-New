...
class EulerAncestralDiscreteScheduler : public Scheduler {
 public:
  EulerAncestralDiscreteScheduler(int num_train_timesteps, float beta_start,
    ...
    // Initialize betas - use double precision for cumulative product to avoid
    // catastrophic precision loss over 1000 steps
    ...
    if (beta_schedule == "linear") {
      ...
      betas_d = xt::pow(xt::linspace<double>(beta_start_sqrt, beta_end_sqrt,
                                              num_train_timesteps),
                        2.0);
    } else if (beta_schedule == "squaredcos_cap_v2") {
      betas_ = betas_for_alpha_bar(num_train_timesteps);
      betas_d = xt::cast<double>(betas_);
    } else {
    ...
    if (rescale_betas_zero_snr_) {
      ...
      alphas_cumprod_d(alphas_cumprod_d.size() - 1) = std::pow(2.0, -24.0);
    }

    // Calculate sigmas from double-precision cumprod
